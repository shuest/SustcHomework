...
#define MAXRLEN 18        
unsigned char status ,snr, Mfrc522_IDBuf[16], TagType[2], SelectedSnr[4];
          ...
          status = PcdAuthState(KEYA, (snr*4+3), DefaultKey, SelectedSnr); 
					{
						if(!status)
						{
							status = PcdRead((snr*4+block), Mfrc522_IDBuf);  //
							if(!status)
							{
                printf("%0x",Mfrc522_IDBuf); //  UART_Send_Char_array(buf,16);
// 								LED_ON4;	
								WaitCardOff();
							}
						}
